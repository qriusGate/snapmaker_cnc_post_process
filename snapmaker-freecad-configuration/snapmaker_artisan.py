...
real GCode suitable for a snapmaker 3 axis mill. This postprocessor, once placed
in the appropriate PathScripts/Macros folder, can be used directly from inside
FreeCAD, via the GUI importer or via python scripts with:

import snapmaker_artisan_post
snapmaker_artisan_post.export(object,"/path/to/file.cnc","")
"""

now = datetime.datetime.now()

parser = argparse.ArgumentParser(prog='snapmaker_artisan', add_help=False)
...
MACHINE_NAME = "Snapmakerartisan"
CORNER_MIN = {'x': 0, 'y': 0, 'z': 0}
CORNER_MAX = {'x': 400, 'y': 400, 'z': 400}
PRECISION = 3
...
def parse(pathobj):
    ...
    # snapmaker doesn't want K properties on XY plane  Arcs need work.
            ...
            for param in params:
                if param in c.Parameters:
                    if param == 'F' and (currLocation[param] != c.Parameters[param] or OUTPUT_DOUBLES):
                        if c.Name not in ["G0", "G00"]:  # snapmaker doesn't use rapid speeds
